# 🚀 IA Generativa aplicada à Educação Financeira  
### 📓 Caderno Temático com NotebookLM  
**Autor:** Caio Branco

---

## 💡 Sobre o Projeto

Este projeto foi desenvolvido como parte de um desafio prático da DIO, com o objetivo de explorar o uso da Inteligência Artificial como ferramenta de aprendizagem ativa.

A proposta consistiu na criação de um caderno temático utilizando o NotebookLM, reunindo fontes confiáveis sobre educação financeira e aplicando técnicas de engenharia de prompts para extrair conhecimento estruturado.

---

## 🎯 Objetivo do Estudo

Investigar como a IA Generativa pode auxiliar no aprendizado de conceitos financeiros básicos, especialmente para iniciantes.

Principais metas:
- 📌 Entender fundamentos de educação financeira  
- 🤖 Utilizar IA para gerar explicações e resumos  
- 🧠 Desenvolver pensamento crítico  
- 🛠️ Praticar engenharia de prompts  

---

## 📚 Fontes Utilizadas

- https://www.bcb.gov.br/cidadaniafinanceira  
- https://www.gov.br/cvm/pt-br/assuntos/educacao-financeira   
- https://www.anbima.com.br/pt_br/educar  
- https://www.investopedia.com/terms/f/financial-literacy.asp  

---

## 🧠 Engenharia de Prompts

### 🔹 Caso 1

**Prompt inicial:**
> O que é educação financeira?

**Resposta da IA:**
> Educação financeira é o conhecimento sobre como lidar com dinheiro.

**Problema:** resposta muito genérica

**Prompt melhorado:**
> Explique o que é educação financeira com exemplos práticos do dia a dia de um jovem.

**Resposta melhorada:**
> Educação financeira é a habilidade de administrar dinheiro de forma consciente, como evitar gastar todo o salário no início do mês, criar uma reserva de emergência e planejar compras maiores.

---

### 🔹 Caso 2

**Prompt inicial:**
> Quais são os principais conceitos financeiros?

**Problema:** resposta vaga

**Prompt melhorado:**
> Liste os 5 principais conceitos financeiros para iniciantes e explique cada um em até 3 linhas.

**Resposta melhorada:**
- Orçamento: controle de receitas e despesas  
- Investimento: aplicação para gerar retorno  
- Juros compostos: crescimento ao longo do tempo  
- Liquidez: facilidade de resgatar dinheiro  
- Risco: possibilidade de perda  

---

## 🩹 Aprendizados

- Respostas genéricas exigem prompts mais específicos  
- Pedir exemplos melhora a compreensão  
- Limitar o formato gera respostas mais organizadas  

---

## 📘 Miniguia de Estudo

### 📝 Resumo

- Educação Financeira: gestão consciente do dinheiro  
- Orçamento: controle financeiro  
- Investimentos: aplicação de recursos  
- Reserva de Emergência: segurança financeira  
- Juros Compostos: crescimento do dinheiro  

---

### 📖 Glossário

| Termo | Definição |
|------|--------|
| Orçamento | Controle financeiro |
| Investimento | Aplicação para retorno |
| Liquidez | Facilidade de saque |
| Risco | Chance de perda |
| Juros Compostos | Juros sobre juros |

---

## 🤖 Prompts Reutilizáveis

- Explique [tema] de forma simples  
- Crie um resumo estruturado  
- Dê exemplos práticos  
- Liste erros comuns  
- Crie um passo a passo  

---

## 🛠️ Ferramentas

- NotebookLM  
- GitHub  
- IA Generativa  

---

## 📌 Conclusão

A IA se mostrou uma ferramenta poderosa para aprendizado ativo, permitindo transformar informações em conhecimento estruturado através de prompts bem elaborados.
